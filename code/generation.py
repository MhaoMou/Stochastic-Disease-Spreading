    ...
    def generate_data(self, num_sample:int, time_horizon:int):
        ...
        for sample_ID in np.arange(0, num_sample, 1):
            ...
            for predicate_idx in np.arange(0, self.num_predicate, 1):
                ...
                data[sample_ID][predicate_idx]['marker'] = [0]
            ...
            for head_predicate_idx in self.head_predicate_set:
                ...
                while t < time_horizon:
                    ...
                    if flag == 1: # accept
                        ...
                        #TODO: sample the marker
                        p = np.random.dirichlet(self.alpha)     #TODO: sample the disease probabilities w.r.t alpha
                        d = np.random.multinomial(n=1,pvals=p)  #TODO: sample the disease type with probabilities p
                        r = np.random.normal(loc=self.mu, scale=self.sigma) #TODO: sample age
                        marker = np.hstack([d,np.array(r)])     #TODO: concatenate
                        data[sample_ID][head_predicate_idx]['marker'].append(marker)
